...
class FluxTransportVectorized:
    ...
    def _advect_temperature(self, dt: float):
        ...
        # Pre-compute thermal mass (ρ*cp) for each material
        thermal_mass = np.zeros((self.state.n_materials,), dtype=np.float32)
        for mat_idx in range(self.state.n_materials):
            props = mat_db.get_properties_by_index(mat_idx)
            thermal_mass[mat_idx] = props.density * props.specific_heat
        
        # Compute energy density E = Σ(φᵢ * ρᵢ * cpᵢ * T)
        energy_density = np.zeros_like(self.state.temperature)
        for mat_idx in range(self.state.n_materials):
            if thermal_mass[mat_idx] > 1.0:  # Skip materials with negligible thermal mass
                energy_density += self.state.vol_frac[mat_idx] * thermal_mass[mat_idx] * self.state.temperature
        ...
        # Compute new thermal mass after material advection
        total_thermal_mass = np.zeros_like(self.state.temperature)
        for mat_idx in range(self.state.n_materials):
            if thermal_mass[mat_idx] > 1.0:
                total_thermal_mass += self.state.vol_frac[mat_idx] * thermal_mass[mat_idx]
        
        # Update temperature: T = E / (Σ φᵢ * ρᵢ * cpᵢ)
        # Only update cells with significant thermal mass
        mask = total_thermal_mass > 1.0
        if np.any(mask):
            self.state.temperature[mask] = energy_density[mask] / total_thermal_mass[mask]
