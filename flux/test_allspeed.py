#!/usr/bin/env python3
"""Test all-speed pressure solver implementation."""

import numpy as np
from simulation import FluxSimulation

def test_simulation(size, scenario="planet", use_allspeed=True):
    """Test simulation with given size."""
    print(f"\n=== Testing {scenario} scenario at {size}x{size} (all-speed={use_allspeed}) ===")
    
    try:
        # Create simulation
        sim = FluxSimulation(nx=size, ny=size, dx=50.0, scenario=scenario)
        
        # Configure physics to use all-speed method
        sim.physics.use_allspeed = use_allspeed
        
        # Force creation of pressure solver before enabling monitoring
        sim.physics.pressure_solver = None  # Reset to force recreation
...
# Test with all-speed method
print("Testing ALL-SPEED method:")
test_simulation(32, use_allspeed=True)
test_simulation(64, use_allspeed=True)
test_simulation(128, use_allspeed=True)

# Compare with standard method
print("\n\nComparing with STANDARD method:")
test_simulation(32, use_allspeed=False)
